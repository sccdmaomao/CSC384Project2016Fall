...
#IMPLEMENT
    ...
    for box in state.boxes or []:
        ...
        for storage in avaliable_storage:
            distance = calc_heuristic_distance(state, box, storage)
            if distance < min_distance:
                min_distance = distance
...
'''
    ...
    Distance is infinit if any of the following condition is satisfied.
        ...
        2. box in corner
'''
def calc_heuristic_distance(state, box, storage):
...
#IMPLEMENT
    ...
    while time.time() - start < timebound: # keep trying different weight while we have time.
        result = se.search(initState=initail_state, heur_fn = heur_alternate, timebound = timebound/timebound, fval_function = fval_function, weight = weight, goal_fn=sokoban_goal_state)
